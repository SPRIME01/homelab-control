// .make_assets/shared-python-lib/generator.ts
import { formatFiles, joinPathFragments, Tree, updateJson } from "@nx/devkit";
import { libraryGenerator as pythonLibraryGenerator } from "@nxlv/python";
import { runCommandsExecutorSchema } from "@nx/devkit/executors";

interface SharedPythonLibGeneratorSchema {
  name: string;
  directory?: string;
  tags?: string;
...
export default async function (
  tree: Tree,
  options: SharedPythonLibGeneratorSchema
) {
  await pythonLibraryGenerator(tree, {
    name: options.name,
